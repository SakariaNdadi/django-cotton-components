...
from django_control_components.studio.models import DashboardSpec, SpecRevision
...
def test_index_scaffolds_from_a_model(client, urlconf, studio_user):
    ...
    assert client.get("/studio/resources/").status_code == 200
    response = client.post("/studio/resources/", {"model": "testapp.author"})
...
def test_builder_renders(client, urlconf, studio_user, spec):
    client.force_login(studio_user)
    response = client.get("/studio/resources/article/")
...
def test_save_columns_and_filters(client, urlconf, studio_user, spec):
    ...
    response = client.post(
        "/studio/resources/article/save/", {"doc": json.dumps(doc), "revision": "0"}
    )
    assert response.status_code == 200
...
def test_preview_renders_a_table(client, urlconf, studio_user, spec, article):
    ...
    response = client.post("/studio/resources/article/preview/", {"doc": json.dumps(doc)})
    assert response.status_code == 200
