...
class Panel:
    ...
    def dynamic(self, value: bool = True) -> Self:
        ...
        ``{panel}/d/<slug>/``. Requires ``django_control_components.studio`` in
        INSTALLED_APPS."""
        self._dynamic = value
        return self
    ...
    def navigation(self, request: HttpRequest) -> list[dict[str, Any]]:
        ...
        if self._dynamic:
            ...
            models = require_studio("models")
            DashboardSpec = models.DashboardSpec
            DynamicResource = require_studio("resource").DynamicResource
            ...
            PanelDashboard = models.PanelDashboard
