...
Requires ``pip install django-control-components[wizard]``.
"""
...
class WizardView(SessionWizardView):  # type: ignore[misc]
    ...
    template_name = "django_control_components/wizards/wizard.html"
    ...
    def as_view(cls, **initkwargs: Any) -> Any:
        if not _HAS_FORMTOOLS:
            raise RuntimeError(
                "WizardView needs django-formtools. Install django-control-components[wizard]."
            )
