...
class Widget:
    ...
    shell_template: ClassVar[str] = "django_cotton_components/panels/widgets/_shell.html"
...
class StatWidget(Widget):
    ...
    def context(self, request: Any) -> dict[str, Any]:
        ...
        if value is None and self._config.get("query"):
            from ..studio.deserialize import resolve_stat_query

            value = resolve_stat_query(self._config["query"])
        else:
...
class ChartWidget(Widget):
    ...
    def _dataset_shape(self, request: Any) -> dict[str, Any]:
        ...
        if raw is None and self._config.get("query"):
            from ..studio.deserialize import resolve_series_query

            raw = resolve_series_query(self._config["query"])
...
class BarListWidget(Widget):
    ...
    template_name = "django_cotton_components/panels/widgets/bar_list.html"
    variant = "bar-list"
    span = 2
